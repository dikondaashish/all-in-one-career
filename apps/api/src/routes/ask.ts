...
export default function askRouter(prisma: PrismaClient, logger: pino.Logger): Router {
  const r = Router();
    ...
    try {
      ...
      try {
        ...
        if (applications.length > 0) {
          userData += `Applications: ${applications.map(app => 
        ...
        if (referrals.length > 0) {
          userData += `Referrals: ${referrals.map(ref => 
            `${ref.role} at ${ref.company} (${ref.status})`
          ).join(', ')}\n`;
        }
        ...
        if (portfolios.length > 0) {
          userData += `Portfolios: ${portfolios.map(p => 
            `${p.slug} (${p.theme})`
          ).join(', ')}\n`;
        }
        ...
        if (jobDescriptions.length > 0) {
          userData += `Job Descriptions: ${jobDescriptions.map(jd => 
...
Analyze the question and respond appropriately. If the question asks for specific items, provide both an answer and indicate what items should be returned.`;
      ...
      let responseType: 'answer' | 'results' | 'both' = 'answer';
